...
$(document).ready(function () {
    ...
    const dispatchGhost = (selectedWall) => {
        ...
        if (timeCounter > 0 || ghostTaunt === true) {
            ...
            // 1 Create a new ghost in the dom
            // 2 Animate the ghost with gsap -> onComplete = break the corresponding wall
            // 3 On Successful click, remove the ghost from the ghost matrix
            ...
            if (currentRing == 2) {
                ghostXYSpawn = 65;
                randCoord = Math.floor(Math.random() * 435) + 145;
                // randCoord = 145 + 435;
            ...
            if (selectedWall == 0) {
                ...
                ghostTimeline.to('.rufus', { duration: 0.5, opacity: randGhostOpacity })
                .to('.rufus', { y: 180, duration: ghostTravelTime, stagger: 0.2, ease: "power2.out", delay: -0.5 });
